...
if (!$viewOnly) {
    $lockStmt = $conn->prepare("UPDATE human_grammarian SET is_locked = 1 WHERE id = ?");
...
// Fetch Document info
$stmt = $conn->prepare("
    SELECT h.id, h.file_path, h.status, h.round, h.phase, h.student_id, h.is_locked,
           s.control_number, h.result_file_path, p.full_name as assigned_personnel
