...
if (!$viewOnly) {
    $lockStmt = $conn->prepare("UPDATE ethics SET is_locked = 1 WHERE id = ?");
...
// Fetch Document info
$stmt = $conn->prepare("
    SELECT e.id, e.file_path, e.status, e.round, e.phase, e.student_id, e.is_locked,
           s.control_number, e.result_file_path, p.full_name as assigned_personnel
    FROM ethics e
...
// 1. Document Paths
$docRelativePath = "/RSSMS/uploads/ethics/" . $submission['file_path'];
$docAbsolutePath = $_SERVER['DOCUMENT_ROOT'] . $docRelativePath;

// 2. Result Paths
$resultFile = $submission['result_file_path'] ?? null;
$resultRelativePath = $resultFile ? "/RSSMS/uploads/ethics_results/" . $resultFile : null;
...
<div class="max-w-6xl mx-auto bg-white dark:bg-warmdark-panel rounded-2xl shadow-lg border border-transparent dark:border-warmdark-border transition-colors duration-200" x-data="{ tab: 'document' }">
    ...
    <div x-show="tab === 'document'" class="p-8" x-cloak>
        <div class="flex justify-between items-center mb-4">
            ...
            <?php if (file_exists($docAbsolutePath)): ?>
