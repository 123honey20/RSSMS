...
<script>
    ...
    function fetchSubmissions(page = 1) {
        ...
        fetch(`../../backend/ajax/fetch_librarian_submissions.php?p=${page}&search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&sy=${encodeURIComponent(sy)}&dept=${encodeURIComponent(pDept)}`)
            .then(res => res.json())
            .then(data => {
