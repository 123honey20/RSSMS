<?php
// Fetch Departments (Specific to Grammarly AI)
...
<div class="bg-white dark:bg-warmdark-panel p-6 rounded-xl shadow-sm min-h-[80vh] border border-transparent dark:border-warmdark-border transition-colors duration-200">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
        <input type="text" id="searchInput" placeholder="Search by Control No..."
        ...
        <table class="w-full text-sm text-left border-collapse text-gray-600 dark:text-gray-300">
            <thead class="bg-gray-50 dark:bg-warmdark-bg text-xs uppercase text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-warmdark-border transition-colors">
                <tr>
                    ...
                    <th class="px-6 py-4 font-semibold text-center">Phase & Round</th> <th class="px-6 py-4 font-semibold">Date & Time</th>
...
<script>
    ...
    function fetchSubmissions(page = 1) {
        ...
        fetch(`../../backend/ajax/fetch_grammarly_ai_submissions.php?p=${page}&search=${encodeURIComponent(search)}&dept=${encodeURIComponent(dept)}&status=${encodeURIComponent(status)}&sy=${encodeURIComponent(sy)}`)
            .then(res => res.json())
            .then(data => {
                ...
                data.submissions.forEach(row => {
                    ...
                    let statusBadge = `<span class="inline-flex items-center justify-center w-[80px] px-2.5 py-1 text-[10px] font-medium uppercase tracking-wide ${badgeColor}">${row.status}</span>`;
                    ...
                    if (row.status === 'Approved' || row.status === 'Needs Revision') {
                        ...
                        actionButton = `<button onclick="loadProcess(${row.id})" class="text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-warmdark-bg border border-gray-200 dark:border-warmdark-border px-4 py-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-warmdark-hover transition font-medium text-xs shadow-sm">Process</button>`;
                    }

                    // ==========================================
                    // DYNAMIC PHASE & ROUND DISPLAY
                    // ==========================================
