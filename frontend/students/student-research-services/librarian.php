...
if ($appStatus === 'Approved') {
    ...
    $subs = $conn->query("SELECT * FROM librarian WHERE student_id = $student_id ORDER BY phase DESC, round DESC, uploaded_at DESC");
    $latestRes = $conn->query("SELECT * FROM librarian WHERE student_id = $student_id ORDER BY phase DESC, round DESC LIMIT 1");
...
<div class="space-y-6 transition-colors duration-200">
    ...
    <?php else: ?>
        ...
        <div class="flex flex-wrap gap-6">
            ...
            <!-- RESULT CARD -->
            <?php if ($isFullyCompleted): ?>
                <a href="student_dashboard.php?page=student_view_librarian_report&id=<?php echo $latest['id']; ?>"
