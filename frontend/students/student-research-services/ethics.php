...
$appStmt = $conn->prepare("
    ...
    WHERE sa.student_id = ? AND sa.service_type = 'Ethics' 
...
if ($appStatus === 'Approved') {
    ...
    $subs = $conn->query("SELECT * FROM ethics WHERE student_id = $student_id ORDER BY phase DESC, round DESC, uploaded_at DESC");
    $latestRes = $conn->query("SELECT * FROM ethics WHERE student_id = $student_id ORDER BY phase DESC, round DESC LIMIT 1");
...
<div class="space-y-6 transition-colors duration-200">
    ...
    <?php else: ?>
        ...
        <div class="flex flex-wrap gap-6">
            ...
            <?php if ($isFullyCompleted): ?>
                <a href="student_dashboard.php?page=student_ethics_approved_result&id=<?php echo $latest['id']; ?>"
        ...
        <div class="bg-white dark:bg-warmdark-panel rounded-lg shadow border border-transparent dark:border-warmdark-border p-6 transition-colors">
            <h2 class="text-md font-semibold text-gray-800 dark:text-gray-100 mb-4">
                History of All File Submit in Ethics
            </h2>
