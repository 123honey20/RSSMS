...
$stmt = $conn->prepare("
    ...
    LEFT JOIN personnel p ON p.id = COALESCE(l.personnel_id, sa.assigned_personnel_id)
...
$stmtComments = $conn->prepare("
    SELECT * FROM librarian_comments
    WHERE librarian_id = ?
...
<div class="max-w-7xl mx-auto space-y-6 transition-colors duration-200">
    ...
    <div class="bg-white dark:bg-warmdark-panel rounded-lg shadow-sm border border-transparent dark:border-warmdark-border p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors">
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Librarian Report Review</h2>
    ...
    <?php if ($resultExists): ?>
        <?php
            ...
            if ($submission['status'] === 'Needs Revision') {
                ...
                $bannerDesc = "Your assigned personnel has uploaded your finalized librarian review result file.";
