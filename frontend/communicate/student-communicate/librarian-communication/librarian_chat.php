...
// FIXED QUERY: Fetch ONLY Approved Personnel for THIS specific service using the Junction Table
                    ...
                    <?php foreach ($personnel_list as $person): ?>
                        <button @click="selectPersonnel(<?php echo $person['personnel_id']; ?>, '<?php echo addslashes(htmlspecialchars($person['full_name'])); ?>')" 
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-gray-900 dark:text-gray-200 truncate transition-colors" :class="unreadCounts[<?php echo $person['personnel_id']; ?>] > 0 ? 'text-black dark:text-white font-extrabold' : 'text-gray-900 dark:text-gray-200'"><?php echo htmlspecialchars($person['full_name']); ?></p>
                                <p class="text-[11px] text-gray-500 dark:text-gray-400 truncate transition-colors" :class="unreadCounts[<?php echo $person['personnel_id']; ?>] > 0 ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 dark:text-gray-400'">
