                    ...
                    <?php foreach ($personnel_list as $person): ?>
                        <button @click="selectPersonnel(<?php echo $person['personnel_id']; ?>, '<?php echo addslashes(htmlspecialchars($person['full_name'])); ?>')" 
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-gray-900 truncate" :class="unreadCounts[<?php echo $person['personnel_id']; ?>] > 0 ? 'text-black font-extrabold' : 'text-gray-900'"><?php echo htmlspecialchars($person['full_name']); ?></p>
                                <p class="text-[11px] text-gray-500 truncate" :class="unreadCounts[<?php echo $person['personnel_id']; ?>] > 0 ? 'text-blue-600 font-bold' : 'text-gray-500'">
            ...
            .then(data => {
                if(data.success) {
                    ...
                    if (scrollToBottom || isNewMessage) {
                        this.scrollToBottom();
                        if (isNewMessage) this.markAsRead();
                    }
                }
            });
        },
