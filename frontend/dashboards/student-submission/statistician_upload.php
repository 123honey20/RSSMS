...
$sub = $conn->query("SELECT * FROM statistician WHERE student_id = $student_id ORDER BY phase DESC, round DESC LIMIT 1");
...
if ($status === 'Pending' && $is_locked === 1) {
    ...
    echo "<script>window.location.href='student_dashboard.php?page=students_rs_statistician';</script>";
    exit();
}
