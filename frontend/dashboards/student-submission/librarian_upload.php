...
$reqStmt = $conn->prepare("SELECT required_phases FROM course_service_requirements WHERE course_id = ? AND service_type = 'Librarian'");
...
if ($status === 'Pending' && $is_locked === 1) {
    ...
    echo "<script>window.location.href='student_dashboard.php?page=students_rs_librarian';</script>";
    exit();
}
...
<div class="max-w-4xl mx-auto py-8 px-4 w-full transition-colors duration-200">
    ...
    <div class="bg-white dark:bg-warmdark-panel rounded-2xl shadow-sm border border-transparent dark:border-warmdark-border p-8 transition-colors">
        ...
        <form action="../../backend/actions/upload-submission/upload_librarian.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm(event)">
