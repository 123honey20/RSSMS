...
$safe_sy = $conn->real_escape_string($active_sy); // Prevent SQL injection

// 1. Fetch Document Submission Data (Filtered by Active SY)
...
$rec_status = ['No Receipt' => 0, 'Receipt Uploaded' => 0, 'Approved' => 0, 'Needs Revision' => 0];
$total_receipts = 0;
...
if ($rec_query) {
    while ($row = $rec_query->fetch_assoc()) {
        if (isset($rec_status[$row['status']])) {
            $rec_status[$row['status']] = (int)$row['count'];
        }
        $total_receipts += (int)$row['count'];
    }
}

// 4. Fetch Feedback Totals (Ratings vs Written Comments)
$total_ratings = 0;
$total_comments = 0;
$fb_stats_query = $conn->query("
    SELECT 
        COUNT(DISTINCT e.id) as total_ratings,
        SUM(CASE WHEN e.comments IS NOT NULL AND TRIM(e.comments) != '' THEN 1 ELSE 0 END) as total_comments
    FROM student_evaluations e
    JOIN students s ON e.student_id = s.id
    WHERE e.service_type LIKE '%Grammarly%' AND s.school_year = '$safe_sy'
");
if ($fb_stats_query) {
...
$fb_query = $conn->query("
    ...
    FROM (
        SELECT e.id, ROUND(AVG(r.score)) as rating
        FROM student_evaluations e
        JOIN student_evaluation_ratings r ON e.id = r.evaluation_id
        JOIN students s ON e.student_id = s.id
        WHERE e.service_type LIKE '%Grammarly%' AND s.school_year = '$safe_sy'
        GROUP BY e.id
    ) as subquery
...
$recent_query = $conn->query("
    SELECT s.control_number, s.thesis_title, g.status, g.round, g.uploaded_at
    FROM grammarly_ai g 
    JOIN students s ON g.student_id = s.id 
    WHERE s.school_year = '$safe_sy'
...
<div class="space-y-6 transition-colors duration-200">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-5 border-b border-gray-200 dark:border-warmdark-border">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Grammarly & AI Analytics</h2>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1">Comprehensive overview of submissions, receipts, and feedback for the current school year.</p>
        </div>
        
        <a href="../dashboards/admin_dashboard.php?page=grammarly_ai_personnel_workload" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div class="bg-white dark:bg-warmdark-panel p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-warmdark-border hover:shadow-md hover:border-blue-100 dark:hover:border-blue-900/50 transition-all duration-300 flex items-center justify-between group">
            <div>
                <p class="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Total Documents</p>
                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 tracking-tight"><?= $total_docs ?></p>
            </div>
        ...
        <div class="bg-white dark:bg-warmdark-panel p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-warmdark-border hover:shadow-md hover:border-yellow-200 dark:hover:border-yellow-900/50 transition-all duration-300 flex items-center justify-between group">
            <div>
                <p class="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Document Pending Review</p>
                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 tracking-tight"><?= $doc_status['Pending'] ?></p>
            </div>
        ...
        <div class="bg-white dark:bg-warmdark-panel p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-warmdark-border hover:shadow-md hover:border-purple-200 dark:hover:border-purple-900/50 transition-all duration-300 flex items-center justify-between group">
            <div>
                <p class="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Receipts Pending</p>
                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 tracking-tight"><?= $rec_status['Receipt Uploaded'] ?></p>
            </div>
            <div class="w-12 h-12 rounded-xl bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 flex items-center justify-center group-hover:bg-purple-600 group-hover:text-white transition-colors duration-300 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="bg-white dark:bg-warmdark-panel p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-warmdark-border hover:shadow-md hover:border-green-200 dark:hover:border-green-900/50 transition-all duration-300 flex items-center justify-between group">
            <div class="flex-1">
                ...
                <div class="flex items-center gap-4">
                    <div>
                        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100 leading-none tracking-tight"><?= $total_ratings ?></p>
                        <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold mt-1">Ratings</p>
                    </div>
                    <div class="w-px h-8 bg-gray-200 dark:bg-warmdark-border"></div>
                    <div>
                        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100 leading-none tracking-tight"><?= $total_comments ?></p>
                        <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold mt-1">Reviews</p>
                    </div>
                </div>
            </div>
            <div class="w-12 h-12 rounded-xl bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-500 flex items-center justify-center group-hover:bg-green-600 group-hover:text-white transition-colors duration-300 shrink-0 ml-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.518 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.978 2.89a1 1 0 00-.364 1.118l1.518 4.674c.3.921-.755 1.688-1.54 1.118l-3.978-2.89a1 1 0 00-1.176 0l-3.978 2.89c-.784.57-1.838-.197-1.539-1.118l1.518-4.674a1 1 0 00-.364-1.118L2.98 9.4A1 1 0 013.568 7.6h4.915a1 1 0 00.95-.69l1.518-4.674z" />
                </svg>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-warmdark-panel p-6 rounded-xl shadow-sm border border-gray-100 dark:border-warmdark-border flex flex-col items-center transition-colors">
            <h3 class="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider self-start mb-4">Document Status</h3>
        ...
        <div class="bg-white dark:bg-warmdark-panel p-6 rounded-xl shadow-sm border border-gray-100 dark:border-warmdark-border flex flex-col items-center transition-colors">
            <h3 class="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider self-start mb-4">Receipt Status</h3>
            <div class="relative w-full max-w-[220px] aspect-square">
    ...
    <div class="bg-white dark:bg-warmdark-panel rounded-xl shadow-sm border border-gray-100 dark:border-warmdark-border overflow-hidden transition-colors">
        <div class="p-5 border-b border-gray-100 dark:border-warmdark-border transition-colors">
            <h3 class="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Recent Document Submissions</h3>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // 1. Document Status (Circle/Donut Graph)
        new Chart(document.getElementById('docStatusChart').getContext('2d'), {
            type: 'doughnut',
            data: {
        ...
        // 2. Receipt Status (Circle/Donut Graph)
