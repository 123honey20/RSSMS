...
$pr_counts = [];
$pr_query = $conn->query("
    SELECT COALESCE(l.phase, 1) as phase, l.round, COUNT(*) as count 
    FROM librarian l 
    JOIN students s ON l.student_id = s.id 
    WHERE s.school_year = '$safe_sy' 
    GROUP BY phase, l.round
    ORDER BY phase ASC, l.round ASC
");
...
$recent_query = $conn->query("
    SELECT s.control_number, s.thesis_title, l.status, l.round, COALESCE(l.phase, 1) as phase, l.uploaded_at
    FROM librarian l 
    JOIN students s ON l.student_id = s.id 
    WHERE s.school_year = '$safe_sy'
