...
$pr_counts = [];
$pr_query = $conn->query("
    SELECT COALESCE(e.phase, 1) as phase, e.round, COUNT(*) as count 
    FROM ethics e 
    JOIN students s ON e.student_id = s.id 
    WHERE s.school_year = '$safe_sy' 
    GROUP BY phase, e.round
    ORDER BY phase ASC, e.round ASC
");
...
// 3. Fetch Feedback Totals (Ratings vs Written Comments)
$total_ratings = 0;
$total_comments = 0;
$fb_stats_query = $conn->query("
    ...
    WHERE ev.service_type LIKE '%Ethics%' AND s.school_year = '$safe_sy'
");
if ($fb_stats_query) {
...
// 3b. Fetch Feedback Ratings Distribution (DYNAMIC based on rubric levels)
$fb_labels = [];
$fb_counts = [];
$fb_query = $conn->query("
...
// 4. Fetch Recent Submissions (Filtered by Active SY)
$recent_query = $conn->query("
    SELECT s.control_number, s.thesis_title, e.status, e.round, e.phase, e.uploaded_at
    FROM ethics e 
    JOIN students s ON e.student_id = s.id 
    WHERE s.school_year = '$safe_sy'
...
<div class="space-y-6 transition-colors duration-200">
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-200 dark:border-warmdark-border">
        <div>
            <div class="flex items-center gap-3">
                <h2 class="text-2xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">Ethics Analytics</h2>
        ...
        <a href="../dashboards/admin_dashboard.php?page=ethics_personnel_workload" 
    ...
    <div class="bg-white dark:bg-warmdark-panel rounded-xl shadow-sm border border-gray-100 dark:border-warmdark-border overflow-hidden transition-colors">
        <div class="p-5 border-b border-gray-100 dark:border-warmdark-border">
            <h3 class="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Recent Ethics Submissions</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                <thead class="bg-gray-50 dark:bg-warmdark-bg text-xs uppercase text-gray-500 dark:text-gray-400 border-b border-transparent dark:border-warmdark-border">
                    <tr>
                        <th class="px-6 py-3 font-semibold">Control No.</th>
                        <th class="px-6 py-3 font-semibold">Thesis Title</th>
                        <th class="px-6 py-3 font-semibold text-center">Phase & Round</th>
                    ...
                    <?php if($recent_query && $recent_query->num_rows > 0): ?>
                        <?php while($row = $recent_query->fetch_assoc()): ?>
                            <tr class="hover:bg-gray-50 dark:hover:bg-warmdark-hover transition-colors">
                                ...
                                <td class="px-6 py-3 text-center font-medium text-gray-600 dark:text-gray-300">
                                    <div class="flex flex-col">
                                        <span class="text-xs">Phase <?= $row['phase'] ?? 1 ?></span>
                                        <span class="text-[10px] text-gray-400">Round <?= $row['round'] ?></span>
                                    </div>
                                </td>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="5" class="px-6 py-8 text-center text-gray-400 dark:text-gray-500">No recent document submissions for this school year.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // We use a semi-transparent gray that looks good on BOTH light and dark backgrounds for the grid lines
    ...
    // 2. Feedback Ratings (Horizontal Bar Graph - DYNAMICALLY RENDERED SCORES)
    ...
    // 3. Submissions Per Phase & Round (Building Bar Graph - DYNAMICALLY RENDERED)
