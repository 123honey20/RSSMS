...
// 3b. Fetch Feedback Ratings Distribution (1-5 Stars)
...
$avg_rating = $total_ratings > 0 ? number_format($fb_sum / $total_ratings, 1) : "0.0";

// 4. Fetch Recent Submissions (Filtered by Active SY)
$recent_query = $conn->query("
    SELECT s.control_number, s.thesis_title, e.status, e.round, e.uploaded_at
    FROM ethics e 
    JOIN students s ON e.student_id = s.id 
    WHERE s.school_year = '$safe_sy'
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 2. Feedback Ratings (Horizontal Bar Graph)
    ...
    // 3. Submissions Per Round (Building Bar Graph - Expanded to 7)
    new Chart(document.getElementById('roundBuildingChart').getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: '#1e3a8a', // Keeping it dark blue, fits both themes well
