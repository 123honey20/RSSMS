...
$pr_counts = [];
$pr_query = $conn->query("
    SELECT COALESCE(h.phase, 1) as phase, h.round, COUNT(*) as count 
    FROM human_grammarian h 
    JOIN students s ON h.student_id = s.id 
    WHERE s.school_year = '$safe_sy' 
    GROUP BY phase, h.round
    ORDER BY phase ASC, h.round ASC
");
...
$recent_query = $conn->query("
    SELECT s.control_number, s.thesis_title, h.status, h.round, COALESCE(h.phase, 1) as phase, h.uploaded_at
    FROM human_grammarian h 
    JOIN students s ON h.student_id = s.id 
    WHERE s.school_year = '$safe_sy'
...
<div class="space-y-6 transition-colors duration-200">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-5 border-b border-gray-200 dark:border-warmdark-border">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Human Grammarian Analytics</h2>
        ...
        <a href="../dashboards/admin_dashboard.php?page=human_grammarian_personnel_workload" 
    ...
    <div class="bg-white dark:bg-warmdark-panel rounded-xl shadow-sm border border-gray-100 dark:border-warmdark-border overflow-hidden transition-colors">
        <div class="p-5 border-b border-gray-100 dark:border-warmdark-border transition-colors">
            <h3 class="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Recent Human Grammarian Submissions</h3>
        </div>
