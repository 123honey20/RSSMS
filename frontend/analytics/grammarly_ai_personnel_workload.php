...
<div class="space-y-6 transition-colors duration-200" x-data="personnelWorkloadApp()">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-5 border-b border-gray-200 dark:border-warmdark-border">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <a href="admin_dashboard.php?page=grammarly_ai_analytics" class="text-gray-400 hover:text-blue-600 dark:text-gray-500 dark:hover:text-blue-400 transition-colors p-1 rounded-md hover:bg-gray-100 dark:hover:bg-warmdark-hover">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 ml-9 font-medium">Monitor evaluation metrics, document backlogs, and pending receipts for Grammarly & AI reviewers.</p>
        </div>
    ...
    <div x-show="selectedDept && !isLoading" x-transition.opacity x-cloak class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-warmdark-panel rounded-2xl shadow-sm border border-gray-200 dark:border-warmdark-border overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col group">
                ...
                <div class="p-5 border-b border-gray-100 dark:border-warmdark-border flex items-center justify-between bg-gray-50/50 dark:bg-warmdark-bg/50">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <h3 class="text-xs font-bold text-gray-900 dark:text-gray-100 truncate" :title="person.full_name" x-text="person.full_name"></h3>
                ...
                <div class="p-5 flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-2">
                        ...
                        <div class="border border-gray-100 dark:border-warmdark-border rounded-lg p-3 bg-white dark:bg-warmdark-panel relative overflow-hidden">
                            ...
                            <p class="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-0.5 ml-1">Doc Backlog</p>
        ...
        <div x-show="personnelList.length === 0" class="col-span-full py-20 text-center bg-gray-50 dark:bg-warmdark-bg rounded-2xl border border-dashed border-gray-300 dark:border-warmdark-border">
            ...
            <p class="text-sm text-gray-500 font-medium">No Grammarly & AI personnel have been mapped to this department.</p>
        </div>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm" x-cloak>
        <div @click.away="closeModal()" class="bg-white dark:bg-warmdark-panel rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] border border-gray-200 dark:border-warmdark-border transform transition-all">
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 mt-1">
                            ...
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">• Grammarly & AI Checking Reviewer</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-8 overflow-y-auto custom-scrollbar bg-white dark:bg-warmdark-panel">
                        ...
                        <div class="w-full">
                            ...
                            <div class="flex justify-between items-end mb-1.5">
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">Document Clearance Rate</span>
                ...
                <h4 class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-4">Caseload Breakdown (Including Receipts)</h4>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="bg-white dark:bg-warmdark-panel border-t-4 border-t-amber-500 border border-gray-200 dark:border-warmdark-border rounded-xl p-5 shadow-sm relative overflow-hidden">
                        <p class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Doc Backlog (Pending)</p>
                    ...
                    <div class="bg-white dark:bg-warmdark-panel border-t-4 border-t-purple-500 border border-gray-200 dark:border-warmdark-border rounded-xl p-5 shadow-sm relative overflow-hidden">
                        <p class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Receipts Pending</p>
                        <p class="text-3xl font-extrabold text-purple-600 dark:text-purple-400" x-text="selectedPerson ? selectedPerson.total_pending_receipts : 0"></p>
                    </div>
    ...
    <div x-show="studentsModalOpen" class="fixed inset-0 z-[110] flex items-center justify-center p-8 sm:p-6 bg-slate-900/80 backdrop-blur-sm" x-cloak>
...
function personnelWorkloadApp() {
    return {
        ...
        fetchPersonnelStats() {
            ...
            fetch(`../../backend/ajax/fetch_grammarly_ai_personnel_stats.php?dept_id=${this.selectedDept}`)
                .then(res => res.json())
                .then(data => {
        ...
        getInsightText(person) {
            ...
            let pendingDocs = parseInt(person.total_pending);
            let pendingReceipts = parseInt(person.total_pending_receipts);
            let combinedBacklog = pendingDocs + pendingReceipts;
            
            if (total === 0) return "This personnel currently has no assigned students for this school year.";
            if (combinedBacklog > 15) return `High Workload Alert: This reviewer has a massive backlog of ${pendingDocs} documents and ${pendingReceipts} receipts waiting. Consider reassigning new students to other personnel.`;
            if (combinedBacklog > 0) return `This reviewer is actively processing. They have ${pendingDocs} documents and ${pendingReceipts} receipts currently waiting for evaluation.`;
            
            return "This reviewer has cleared their entire document and receipt backlog! They are currently free to take on new assignments.";
        },
        ...
        getAvailability(person) {
            ...
            // GRAMMARLY SPECIFIC: Combine pending docs AND pending receipts
            let pending = parseInt(person.total_pending || 0) + parseInt(person.total_pending_receipts || 0);
