...
<div class="space-y-6 transition-colors duration-200" x-data="personnelWorkloadApp()">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-5 border-b border-gray-200 dark:border-warmdark-border">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <a href="admin_dashboard.php?page=statistician_analytics" class="text-gray-400 hover:text-blue-600 dark:text-gray-500 dark:hover:text-blue-400 transition-colors p-1 rounded-md hover:bg-gray-100 dark:hover:bg-warmdark-hover">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 ml-9 font-medium">Monitor evaluation metrics and active caseloads for Statistician reviewers.</p>
        </div>
    ...
    <div x-show="selectedDept && !isLoading" x-transition.opacity x-cloak class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div x-show="personnelList.length === 0" class="col-span-full py-20 text-center bg-gray-50 dark:bg-warmdark-bg rounded-2xl border border-dashed border-gray-300 dark:border-warmdark-border">
            ...
            <p class="text-sm text-gray-500 font-medium">No Statistician personnel have been mapped to this department.</p>
        </div>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm" x-cloak>
        <div @click.away="closeModal()" class="bg-white dark:bg-warmdark-panel rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] border border-gray-200 dark:border-warmdark-border transform transition-all">
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 mt-1">
                            ...
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">• Statistician</span>
                        </div>
                    </div>
                </div>
...
function personnelWorkloadApp() {
    return {
        ...
        fetchPersonnelStats() {
            ...
            fetch(`../../backend/ajax/fetch_statistician_personnel_stats.php?dept_id=${this.selectedDept}`)
                .then(res => res.json())
                .then(data => {
