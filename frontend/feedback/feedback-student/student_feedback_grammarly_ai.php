...
$stmt = $conn->prepare("
    SELECT ga.id as submission_id, s.control_number, p.id as personnel_id, p.full_name as personnel_name
    FROM grammarly_ai ga
    JOIN students s ON ga.student_id = s.id
    LEFT JOIN personnel p ON ga.personnel_id = p.id
...
<div class="max-w-5xl mx-auto pb-12 pt-6 transition-colors duration-200" x-data="studentEvaluationForm(<?php echo htmlspecialchars($jsonRubrics, ENT_QUOTES, 'UTF-8'); ?>)">
    ...
    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Provide feedback for your recent Grammarly & AI Checking.</p>
        </div>
                ...
                fetch('../../backend/ajax/save_student_feedback.php', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            setTimeout(() => {
                                window.location.href = 'student_dashboard.php?page=student_grammarly_ai_approved_result&id=<?php echo $submissionId; ?>';
                            }, 3000);
                        } else {
