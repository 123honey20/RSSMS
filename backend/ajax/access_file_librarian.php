...
// 1. Fetch the actual personnel ID and info from the personnel table
...
// 2. FETCH DETAILED INFO: JOINs for departments and thesis_title
...
// 3. Update status AND lock this submission to THIS personnel_id
$stmt = $conn->prepare("UPDATE librarian SET status = ?, personnel_id = ? WHERE id = ?");
...
if ($success && $info) {
    try {
        ...
        $footer = "<div style='background:#f1f5f9;padding:20px;text-align:center;font-size:12px;color:#64748b;'><p style='margin:0;'>This is an automated system log from RSSMS.</p></div></div></div>";
        ...
        // --- Personnel Email (Table UI) ---
        $mail->clearAddresses();
        $mail->addAddress($personnel_email, $personnel_name);
        $mail->Subject = "Action Logged: Librarian Review Complete - $status";
        ...
        $mail->Body = $header . "
            ...
            <div style='padding:30px;line-height:1.6;color:#334155;'>
                ...
                <p>This email confirms that you have processed a <strong>Librarian Review</strong> submission. Summary:</p>
                ...
                <div style='background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:20px; margin:20px 0;'>
                    ...
                    <table style='width:100%; font-size:14px; border-collapse:collapse;'>
                        <tr><td style='padding:8px 0; color:#64748b;'>Student Leader:</td><td style='font-weight:bold;'>{$info['research_leader']}</td></tr>
                        ...
                        <tr>
                            <td style='padding:15px 0 8px 0; color:#64748b;'>Final Result:</td>
                            <td style='padding:15px 0 8px 0;'><span style='background:$themeColor; color:#fff; padding:4px 10px; border-radius:4px; font-weight:bold; font-size:12px; text-transform:uppercase;'>$status</span></td>
                        </tr>
                    </table>
                </div>
                <p style='font-size:13px; color:#64748b;'>The student has been notified via email.</p>
            </div>" . $footer;
        $mail->send();
