...
// 2. Fetch all Ethics Personnel assigned to this department
$personnel_data = [];

// Use EXACT MATCH for Ethics
$pStmt = $conn->prepare("
...
while ($person = $pRes->fetch_assoc()) {
    ...
    // Metric A: Total Students Assigned
    $assignQuery = $conn->query("
        SELECT COUNT(*) as total
        FROM service_applications sa
        JOIN students s ON sa.student_id = s.id
        WHERE sa.assigned_personnel_id = $p_id AND sa.service_type = 'Ethics' AND s.school_year = '$safe_sy'
    ");
    ...
    $studentsQuery = $conn->query("
        SELECT s.control_number, s.research_leader, s.thesis_title,
               (sa.assigned_personnel_id != COALESCE(
                   (SELECT rl.from_personnel_id FROM reassignment_logs rl WHERE rl.student_id = s.id AND rl.service_type = 'Ethics' ORDER BY rl.reassigned_at ASC LIMIT 1),
                   sa.assigned_personnel_id
               )) as is_sub,
               COALESCE(
                   (SELECT MAX(rl.reassigned_at) FROM reassignment_logs rl WHERE rl.student_id = s.id AND rl.service_type = 'Ethics' AND rl.to_personnel_id = sa.assigned_personnel_id),
                   sa.updated_at,
                   sa.created_at
               ) as assigned_date,
               COALESCE(eth.status, 'Pending') as current_status
        FROM service_applications sa
        JOIN students s ON sa.student_id = s.id
        LEFT JOIN (
        ...
        LEFT JOIN ethics eth ON eth.id = latest.max_id
        WHERE sa.assigned_personnel_id = $p_id AND sa.service_type = 'Ethics' AND s.school_year = '$safe_sy'
    ...
    // Metric B: Document Status Breakdown
    $statusQuery = $conn->query("
        ...
        LEFT JOIN course_service_requirements csr ON s.course_id = csr.course_id AND csr.service_type = 'Ethics'
        WHERE sa.assigned_personnel_id = $p_id AND sa.service_type = 'Ethics' AND s.school_year = '$safe_sy'
    ...
    // Combine data (Rating entirely removed)
