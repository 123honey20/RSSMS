...
if (!isset($_SESSION['service_role']) || strtolower(trim($_SESSION['service_role'])) !== 'statistician') {
    echo json_encode(["submissions" => [], "totalPages" => 0, "currentPage" => 1, "totalRows" => 0]); exit;
}
...
// 3. CRITICAL UPDATE: Lock the query to ONLY show submissions if this specific personnel was assigned to the student
$where = "WHERE sa.service_type = 'Statistician' AND sa.status = 'Approved' AND sa.assigned_personnel_id = ?";
$params = [$personnel_id];
$types = "i";
...
// Update Count Query to JOIN the service_applications table
$countSql = "
...
$sql = "
    SELECT t.id, t.status, t.round, t.phase, t.uploaded_at, t.updated_at, s.control_number, s.research_leader, s.thesis_title, u.email, d.name AS department_name, c.name AS course_name,
    ...
    JOIN service_applications sa ON sa.student_id = s.id
    LEFT JOIN course_service_requirements csr ON s.course_id = csr.course_id AND csr.service_type = 'Statistician'
    $where
