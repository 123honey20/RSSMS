...
if (empty($allowed_depts)) {
    ...
    if ($target_dept !== 'All' && !empty($target_dept)) {
        ...
        if (in_array($target_dept, $allowed_depts)) {
            ...
            // Unauthorized access attempt (they tried to filter a dept they don't own)
            $where .= " AND s.department_id = 0"; 
        }
    } else {
...
$countSql = "SELECT COUNT(*) as total FROM statistician t JOIN students s ON t.student_id = s.id $where";
