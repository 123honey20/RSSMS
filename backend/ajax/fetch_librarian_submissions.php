...
if (!isset($_SESSION['service_role']) || strtolower(trim($_SESSION['service_role'])) !== 'librarian') {
    echo json_encode(["submissions" => [], "totalPages" => 0, "currentPage" => 1, "totalRows" => 0]); exit;
}
...
// Update Main Query - FIX: Added l.phase and max_phases
$sql = "
    SELECT l.id, l.status, l.round, l.phase, s.control_number, s.research_leader, s.thesis_title, u.email, d.name AS department_name, c.name AS course_name,
    ...
    JOIN service_applications sa ON sa.student_id = s.id
    LEFT JOIN department_service_requirements dsr ON s.department_id = dsr.department_id AND dsr.service_type = 'Librarian'
    $where
