...
while ($person = $pRes->fetch_assoc()) {
    ...
    $assignQuery = $conn->query("
        ...
        WHERE sa.assigned_personnel_id = $p_id AND sa.service_type = 'Statistician' AND s.school_year = '$safe_sy'
    ");
    ...
    $studentsQuery = $conn->query("
        SELECT s.control_number, s.research_leader, s.thesis_title,
               (sa.assigned_personnel_id != COALESCE(
                   (SELECT rl.from_personnel_id FROM reassignment_logs rl WHERE rl.student_id = s.id AND rl.service_type = 'Statistician' ORDER BY rl.reassigned_at ASC LIMIT 1),
                   sa.assigned_personnel_id
               )) as is_sub,
               COALESCE(
                   (SELECT MAX(rl.reassigned_at) FROM reassignment_logs rl WHERE rl.student_id = s.id AND rl.service_type = 'Statistician' AND rl.to_personnel_id = sa.assigned_personnel_id),
                   sa.updated_at,
                   sa.created_at
               ) as assigned_date,
               COALESCE(st.status, 'Pending') as current_status
        FROM service_applications sa
        JOIN students s ON sa.student_id = s.id
        LEFT JOIN (
        ...
        LEFT JOIN statistician st ON st.id = latest.max_id
        WHERE sa.assigned_personnel_id = $p_id AND sa.service_type = 'Statistician' AND s.school_year = '$safe_sy'
