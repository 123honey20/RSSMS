...
// Fetch details for richer email
...
// Update status and lock this submission to THIS personnel_id
$stmt = $conn->prepare("UPDATE human_grammarian SET status = ?, personnel_id = ? WHERE id = ?");
...
if ($success && $info) {
    try {
        ...
        $footer = "<div style='background:#f1f5f9;padding:20px;text-align:center;font-size:12px;color:#64748b;'><p style='margin:0;'>Automated System Log.</p></div></div></div>";

        // Student Email
        $mail->addAddress($info['student_email'], $info['research_leader']);
        $mail->Subject = "Human Grammarian Review Result: $status";
        $mail->Body = $header . "<div style='background:$themeColor;padding:30px;text-align:center;'><h1 style='color:#fff;margin:0;font-size:22px;'>Review $status</h1></div><div style='padding:30px;line-height:1.6;color:#334155;'><p>Hello <strong>{$info['research_leader']}</strong>,</p><p>Your document has been $status by the Grammarian.</p></div>" . $footer;
        $mail->send();

        // Personnel Email (Professional Table)
        $mail->clearAddresses();
        ...
        $mail->Body = $header . "
            ...
            <div style='padding:30px;line-height:1.6;color:#334155;'>
                ...
                <div style='background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:20px; margin:20px 0;'>
                    <h3 style='margin-top:0; color:#334155; border-bottom:1px solid #e2e8f0; padding-bottom:10px;'>Submission Summary</h3>
                    <table style='width:100%; font-size:14px;'>
                        <tr><td>Student:</td><td style='font-weight:bold;'>{$info['research_leader']}</td></tr>
                        <tr><td>Dept:</td><td>{$info['dept_name']}</td></tr>
                        <tr><td>Round:</td><td>{$info['round']}</td></tr>
                        <tr><td>Title:</td><td style='font-style:italic;'>{$info['thesis_title']}</td></tr>
                        <tr><td>Result:</td><td><span style='background:$themeColor; color:#fff; padding:2px 8px; border-radius:4px;'>$status</span></td></tr>
                    </table>
                </div>
            </div>" . $footer;
        $mail->send();
