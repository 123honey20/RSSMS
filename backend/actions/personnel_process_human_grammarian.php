    ...
    if ($action === 'Approve' || $action === 'Needs Revision') {
        ...
        if ($has_file) {
            ...
            if (!in_array($ext, $allowed_exts)) {
                ...
                header("Location: ../../frontend/dashboards/personnel_dashboard.php?page=submissions_human_grammarian");
                exit();
            }
            
            $filename = "result_" . time() . "_" . $submission_id . "." . $ext;
            $targetDir = "../../uploads/human_grammarian_results/";
            if (!is_dir($targetDir)) {
        ...
        if ($filename) {
            $stmt = $conn->prepare("UPDATE human_grammarian SET status = ?, result_file_path = ?, personnel_id = ?, updated_at = NOW() WHERE id = ?");
            $stmt->bind_param("ssii", $status, $filename, $actual_personnel_id, $submission_id);
        } else {
            $stmt = $conn->prepare("UPDATE human_grammarian SET status = ?, personnel_id = ?, updated_at = NOW() WHERE id = ?");
                ...
                try {
                    ...
                    $mail->Body = $header . "
                        ...
                        <div style='padding:30px;line-height:1.6;color:#334155;'>
                            ...
                            <p>This email confirms that you have processed a <strong>Human Grammarian</strong> submission review:</p>
