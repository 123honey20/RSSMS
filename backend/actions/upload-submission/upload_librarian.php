...
$assignStmt = $conn->prepare("SELECT assigned_personnel_id, extra_rounds FROM service_applications WHERE student_id = ? AND service_type = 'Librarian' AND status = 'Approved'");
...
$checkStmt = $conn->prepare("SELECT * FROM librarian WHERE student_id = ? ORDER BY phase DESC, round DESC LIMIT 1");
...
if ($latest) {
    ...
    if ($latest['status'] === 'Pending') {
        ...
        // REASSIGNMENT UPDATE
        $stmt = $conn->prepare("UPDATE librarian SET file_path = ?, status = 'Pending', personnel_id = ?, is_locked = 0, uploaded_at = NOW() WHERE id = ?");
        ...
        $round++; 
        $stmt = $conn->prepare("INSERT INTO librarian (student_id, school_id, file_path, status, round, phase, personnel_id, is_locked) VALUES (?, ?, ?, 'Pending', ?, ?, ?, 0)");
        $stmt->bind_param("issiii", $student_id, $school_id, $filename, $round, $phase, $active_personnel_id);
        $stmt->execute();

    } elseif ($latest['status'] === 'Approved') {
        if ($phase >= $max_phases) {
            redirectWithError("You have already completed all required phases for Librarian.", $redirect_url, $targetFile);
        }
        $phase++; 
        $round = 1; 
        $stmt = $conn->prepare("INSERT INTO librarian (student_id, school_id, file_path, status, round, phase, personnel_id, is_locked) VALUES (?, ?, ?, 'Pending', ?, ?, ?, 0)");
...
try {
    ...
    $mail->Subject = "Librarian Review Document Submitted - {$phaseText}Round $round";
    $mail->Body = $header . "
        ...
        <div style='padding:30px;line-height:1.6;color:#334155;'>
            ...
            <p>Your document for <strong>Librarian review</strong> has been uploaded/updated for {$phaseText}Round $round.</p>
    ...
    if ($personnelEmailsFound) {
        $mail->Subject = "ACTION REQUIRED: Librarian Submission {$phaseText}Round $round - $controlNo";
        $mail->Body = $header . "
            ...
            <div style='padding:30px;line-height:1.6;color:#334155;'>
                <p>A research document has been submitted/updated for librarian review from your department.</p>
                <div style='background:#eff6ff; border-left: 4px solid #2563eb; padding: 15px; margin: 20px 0;'>
                    ...
                    <p style='margin:0;'><strong>Phase & Round:</strong> <span style='background:#dcfce7; color:#166534; padding:2px 8px; border-radius:10px; font-weight:bold;'>{$phaseText}Round $round</span></p>
