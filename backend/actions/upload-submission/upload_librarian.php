...
if (move_uploaded_file($file['tmp_name'], $targetFile)) {
    ...
    if ($latest) {
        ...
        if ($latest['status'] === 'Pending') {
            ...
            $stmt = $conn->prepare("UPDATE librarian SET file_path = ?, status = 'Pending', is_locked = 0, uploaded_at = NOW() WHERE id = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO librarian (student_id, school_id, file_path, status, round, phase, is_locked) VALUES (?, ?, ?, 'Pending', ?, ?, 0)");
