...
if (move_uploaded_file($file['tmp_name'], $targetFile)) {
    ...
    if ($latest) {
        ...
        if ($latest['status'] === 'Pending') {
            ...
            $round++; // Advance Round
            $stmt = $conn->prepare("INSERT INTO librarian (student_id, school_id, file_path, status, round, phase, personnel_id) VALUES (?, ?, ?, 'Pending', ?, ?, ?)");
        ...
        $stmt = $conn->prepare("INSERT INTO librarian (student_id, school_id, file_path, status, round, phase) VALUES (?, ?, ?, 'Pending', ?, ?)");
    ...
    try {
        ...
        $mail->Body = $header . "<div style='background:#059669;padding:30px;text-align:center;'><h1 style='color:#fff;margin:0;font-size:24px;'>Document Submitted</h1></div><div style='padding:30px;line-height:1.6;color:#334155;'><p>Hello <strong>$studentName</strong>,</p><p>Your document for <strong>Librarian review</strong> has been uploaded for {$phaseText}Round $round.</p><p><strong>Control No:</strong> $controlNo</p></div>" . $footer;
        $mail->send();
