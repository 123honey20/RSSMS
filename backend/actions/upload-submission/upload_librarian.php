...
if (move_uploaded_file($file['tmp_name'], $targetFile)) {
    ...
    if ($latest) {
        ...
        if ($status === 'Pending') {
            ...
            $stmt = $conn->prepare("UPDATE librarian SET file_path = ?, status = 'Pending' WHERE id = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO librarian (student_id, school_id, file_path, status, round) VALUES (?, ?, ?, 'Pending', 1)");
