...
$redirect_url = "../../../frontend/dashboards/student_dashboard.php?page=student_upload_statistician";
...
// 1. Fetch Student Info INCLUDING course_id
$stmt = $conn->prepare("SELECT s.id, s.control_number, s.research_leader, s.thesis_title, s.department_id, s.course_id, u.email, u.school_id FROM students s JOIN users u ON s.user_id = u.id WHERE u.id = ?");
...
$targetDir = "../../../uploads/statistician/";
...
if ($latest) {
    ...
    if ($latest['status'] === 'Pending') {
        ...
        // REASSIGNMENT UPDATE
        $stmt = $conn->prepare("UPDATE statistician SET file_path = ?, status = 'Pending', personnel_id = ?, is_locked = 0, uploaded_at = NOW() WHERE id = ?");
...
try {
    ...
    $footer = "<div style='background:#f1f5f9;padding:20px;text-align:center;font-size:12px;color:#64748b;'><p>Automated Statistician Notification.</p></div></div></div>";
