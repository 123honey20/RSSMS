...
// Fetch assigned personnel and check document locks
$checkSub = $conn->prepare("SELECT id, file_path, personnel_id, is_locked FROM grammarly_ai WHERE student_id = ? AND round = ? LIMIT 1");
$checkSub->bind_param("ii", $student_id, $round);
$checkSub->execute();
$existingDoc = $checkSub->get_result()->fetch_assoc();
$assigned_personnel_id = $existingDoc['personnel_id'] ?? null;
$checkSub->close();
...
if ($doReceipt) {
    ...
    move_uploaded_file($_FILES['receipt_file']['tmp_name'], $receiptDir . $receiptFilename);
...
if ($doDocument) {
    ...
    move_uploaded_file($_FILES['submission_file']['tmp_name'], $docDir . $docFilename);

    if ($existingDoc) {
        $oldPath = $docDir . $existingDoc['file_path'];
        if (file_exists($oldPath)) unlink($oldPath); 

        // Update document AND reset the lock (is_locked = 0) so personnel knows it's a fresh file!
        $subStmt = $conn->prepare("UPDATE grammarly_ai SET file_path = ?, status = 'Pending', is_locked = 0, uploaded_at = NOW() WHERE id = ?");
        $subStmt->bind_param("si", $docFilename, $existingDoc['id']);
    } else {
        ...
        $subStmt->bind_param("issii", $student_id, $school_id, $docFilename, $round, $assigned_personnel_id);
    }
    $subStmt->execute();
    $subStmt->close();
}
