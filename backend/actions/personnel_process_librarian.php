...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'Approve' || $action === 'Reject') {
        ...
        if ($has_file) {
            ...
            // Define target directory (Unique to librarian)
        ...
        // --- 2. UPDATE DATABASE ---
        if ($filename) {
            $stmt = $conn->prepare("UPDATE librarian SET status = ?, result_file_path = ? WHERE id = ?");
            $stmt->bind_param("ssi", $status, $filename, $submission_id);
        } else {
            $stmt = $conn->prepare("UPDATE librarian SET status = ? WHERE id = ?");
