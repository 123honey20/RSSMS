...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'Approve' || $action === 'Needs Revision') {
        ...
        if ($filename) {
            $stmt = $conn->prepare("UPDATE librarian SET status = ?, result_file_path = ?, personnel_id = ?, updated_at = NOW() WHERE id = ?");
            $stmt->bind_param("ssii", $status, $filename, $actual_personnel_id, $submission_id);
        } else {
            $stmt = $conn->prepare("UPDATE librarian SET status = ?, personnel_id = ?, updated_at = NOW() WHERE id = ?");
